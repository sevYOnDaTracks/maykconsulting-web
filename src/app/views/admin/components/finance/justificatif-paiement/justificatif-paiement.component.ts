...
@Component({
  selector: 'app-justificatif-paiement',
  templateUrl: './justificatif-paiement.component.html',
  styleUrl: './justificatif-paiement.component.scss'
})
export class JustificatifPaiementComponent implements OnInit {
  ...
  constructor(
      private dialogRef: MatDialogRef<JustificatifPaiementComponent>,
      @Inject(MAT_DIALOG_DATA) public data: { financeData: any },
  ...
  ngOnInit(): void {
    ...
    this.fileUrl = this.data.financeData.justificatifPaiement || null;
  }
  ...
  async save(): Promise<void> {
    console.log('hello');
    console.log(this.data.financeData);
    ...
    try {
      if (this.file) {
        this.fileUrl = await this.financeService.uploadDocument(this.file, this.data.financeData.userId, 'justificatifPaiement');
      }
      await this.financeService.updateFinanceData(this.data.financeData.userId, {
      ...
      await this.sendNotificationMailToAdmin(this.data.financeData.city, this.data.financeData.country);
  ...
  async sendNotificationMailToUser() {
    ...
    this.emailService.sendEmailNotificationPaiementRec(this.user.email, 'Paiement reçu - Votre Dossier de Garant Financier ').subscribe(
        response => {
  ...
  async sendNotificationMailToAdmin(city: string , country: string) {
    ...
    this.emailService.sendEmailNotificationPaiementRecAdmin('maykconsulting@gmail.com', 'Paiement Garant - ' + this.user.firstName  + ' ' + this.user.lastName , this.user.firstName  + ' ' + this.user.lastName, city , country).subscribe(
        response => {
          console.log(response);
        },
        error => {
