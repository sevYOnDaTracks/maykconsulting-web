...
@Component({
  selector: 'app-edit-paiement-justificatif',
  templateUrl: './edit-paiement-justificatif.component.html',
  styleUrl: './edit-paiement-justificatif.component.scss'
})
export class EditPaiementJustificatifComponent implements OnInit {
    user$: Observable<User | null>;
    user: any;
    file: File | null = null;
    filePreview: string | ArrayBuffer | null = null;
    fileUrl: string | null = null; // URL du fichier téléchargé
    isSubmitting = false;
    paymentForm: FormGroup;

    constructor(
        private dialogRef: MatDialogRef<EditPaiementJustificatifComponent>,
        @Inject(MAT_DIALOG_DATA) public data: { admissionData: any },
    ...
    ngOnInit(): void {
        ...
        this.fileUrl = this.data.admissionData.justificatifPaiement || null;
  }
  ...
  onFileSelected(event: any): void {
    ...
    async save(): Promise<void> {
        ...
        try {
            if (this.file) {
                this.fileUrl = await this.admissionService.uploadDocument(this.file, this.data.admissionData.userId, 'justificatifPaiement');
            }
            await this.admissionService.updateAdmissionData(this.data.admissionData.userId, {
            ...
            await this.sendNotificationMailToAdmin(this.data.admissionData.admissionType , this.data.admissionData.country);
    ...
    async sendNotificationMailToUser() {
        ...
        this.emailService.sendEmailNotificationPaiementRec(this.user.email, 'Paiement reçu - Votre Dossier d\'admission ').subscribe(
            response => {
    ...
    async sendNotificationMailToAdmin(admissionType, country) {
        console.log(this.user);
        // tslint:disable-next-line:max-line-length
        this.emailService.sendEmailNotificationPaiementRecAdmin('maykconsulting@gmail.com', 'Paiement Admission ' + country + ' - ' + this.user.firstName  + ' ' + this.user.lastName , this.user.firstName  + ' ' + this.user.lastName, admissionType , country).subscribe(
            response => {
                console.log(response);
            },
            error => {
